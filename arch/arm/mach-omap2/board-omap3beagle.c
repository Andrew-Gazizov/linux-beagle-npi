...
static struct stmpe_platform_data stmpe811_data = {
    ...
    .irq_base = 266,//OMAP_GPIO_IRQ(106),
    .ts		= &stmpe811_ts_data,
    .autosleep      = false,
    .autosleep_timeout = 1024,
    //    .get_pendown_state = omap3beagle_stmpe_get_pendown_state,
    .irq_trigger    = IRQF_TRIGGER_FALLING,
};
...
static struct spi_board_info board_spi3_board_info[] = {
    //    #if 1
    {
        //        .modalias    = "nec_2432_spi",/* Представление SPI в пространстве пользователя */
        .modalias    = "lgphilips_lb035q02_panel-spi",/* Представление SPI в пространстве пользователя */
...
static int beagle_enable_lcd(struct omap_dss_device *dssdev)
{
    ...
    if (gpio_is_valid(beagle_config.lcd_pwren)) {
        ...
        int r = gpio_request_one(beagle_config.lcd_pwren, GPIOF_OUT_INIT_HIGH,
                                 "LCD power");
        if (r < 0)
    ...
    printk(KERN_INFO "\nDisabling keyboard backlight....from beagle");
...
static void __init beagle_display_init(void)
{
    ...
    r = gpio_request_one(beagle_config.lcd_pwren, GPIOF_OUT_INIT_HIGH,
                         "LCD power");
    if (r < 0)
        printk(KERN_ERR "Unable to get LCD power enable GPIO\n");
#endif
...
static struct twl4030_bci_platform_data beagle_bci_data;
static int beagle_batt_table[] = {
    /* 0 C*/
...
static struct twl4030_bci_platform_data beagle_bci_data = {
    .battery_tmp_tbl    = beagle_batt_table,
    .tblsize        = ARRAY_SIZE(beagle_batt_table),
};
...
#define OMAP3BEAGLE_STMPE_GPIO 106
static int omap3beagle_stmpe_get_pendown_state(void)
{
    printk("omap3beagle_stmpe_get_pendown_state \n");
    return !gpio_get_value(OMAP3BEAGLE_STMPE_GPIO);
}

static int omap3beagle_stmpe_init(void)
{
    int gpio = OMAP3BEAGLE_STMPE_GPIO;
    int irq = OMAP_GPIO_IRQ(gpio);

    int ret = 0;
    printk(KERN_WARNING "stmpe811_init started gpi_to_irq=%d \n", irq);
    ret = gpio_request(gpio, "stmpe811_pen_down");
    if (ret < 0) {
        printk(KERN_ERR "Failed to request GPIO %d for "
               "stmpe811 pen down IRQ\n", gpio);
        return ret;
    }

    printk(KERN_WARNING " omap3beagle_stmpe_initgpio=%d \n", gpio);

    omap_mux_init_gpio(gpio, OMAP_PIN_INPUT_PULLUP);
    gpio_direction_input(gpio);

    omap_mux_init_signal("cam_d7",
...
#define OMAP3BEAGLE_TWL_GPIO 0
//static int omap3beagle_twl_get_pendown_state(void)
//{
//    printk("omap3beagle_twl_get_pendown_state \n");
//    return !gpio_get_value(OMAP3BEAGLE_TWL_GPIO);
//}

static irqreturn_t omap3beagle_twl_get_pendown_state(int irq, void *data)
{
    printk("omap3beagle_twl_get_pendown_state \n");
...
static int omap3beagle_twl_init(void)
{
    int gpio = OMAP3BEAGLE_TWL_GPIO;
    int irq = OMAP_GPIO_IRQ(gpio);

    int ret = 0;
    //    printk("omap3beagle_twl_init started irq=%d \n", irq);
    //    printk("omap3beagle_twl_init started OMAP_GPIO_IRQ=%d \n", OMAP_GPIO_IRQ(gpio));
    ret = gpio_request(gpio, "twl4030_pen_down");
    if (ret < 0) {
        printk(KERN_ERR "Failed to request GPIO %d for "
               "twl4030 pen down IRQ\n", gpio);
        return ret;
    }
    //    printk("omap3beagle_twl_init gpio=%d \n", gpio);
    ...
    //    request_irq(irq, omap3beagle_twl_get_pendown_state, IRQF_TRIGGER_FALLING,
    //                "123", NULL);

    //    printk("omap3beagle_twl_init irq=%d \n", irq);
...
static int __init omap3_beagle_i2c_init(void)
{
    omap3_pmic_get_config(&beagle_twldata,
                          TWL_COMMON_PDATA_USB | TWL_COMMON_PDATA_BCI /* | TWL_COMMON_PDATA_AUDIO*/,
    ...
    //                          TWL_COMMON_REGULATOR_VDAC | TWL_COMMON_REGULATOR_VPLL2);
    //    omap3_pmic_init("twl4030", &beagle_twldata);
    //    omap3_pmic_init("tps65930", &beagle_twldata);
    ...
    omap3beagle_stmpe_init();
    omap3beagle_twl_init();
    return 0;
}
...
static void __init omap3_beagle_init(void)
{
    ...
    /* TODO: set lcd_driver_name by command line or device tree */
    //    beagle_config.lcd_driver_name = "tfc_s9700rtwv35tr-01b",
    beagle_config.lcd_driver_name = "lgphilips_lb035q02_panel",
            lcd_panel.name = beagle_config.lcd_driver_name;
    if (strcmp(motherb_name, "v4") == 0)
        platform_add_devices(omap3_beagle_devices + 1,
                             ARRAY_SIZE(omap3_beagle_devices) - 1);
