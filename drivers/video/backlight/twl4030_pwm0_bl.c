...
static void pwm0_enable(void)
{
	...
	twl_i2c_write_u8(TWL4030_MODULE_PWM0, MAX_VALUE, TWL_PWM0_OFF);
...
///	u8 r;
	...
	///printk("pwm0_backlight_update_status brightness %d\n", bl->props.brightness);
...
/// на Zkpk2 шим подсветки идет через инвертор
/// при минимальной подсветке duty cycle должен быть максимален,
/// при максимальной - минимален(или выключен)
/// поэтому включение/выключение шима ниже закомментировано
/*
	if (brightness == 0) {
		if (old_brightness != 0)
			pwm0_disable();
...
*/
	twl_i2c_write_u8(TWL4030_MODULE_PWM0, MAX_VALUE - brightness,
				TWL_PWM0_OFF);
...
static int pwm0_backlight_probe(struct platform_device *pdev)
{
	...
	/* 128 cycle period, ON position 0 */
	twl_i2c_write_u8(TWL4030_MODULE_PWM0, 0x00, TWL_PWM0_ON);
