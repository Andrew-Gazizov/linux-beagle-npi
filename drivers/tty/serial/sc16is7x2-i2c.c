...
static int sc16is7x2_read(struct i2c_client *client, unsigned reg, unsigned ch)
{
    ...
    if(reg==0x0)
        printk("   %s (ch%i) reg %x val %x reg_dev %x \n", __func__, ch, reg, ret, read_cmd(reg, ch));

    //    if(!((reg==UART_LSR) && ((ret&0x1)==0x0)))
    //        printk("   %s (ch%i) reg %x val %x reg_dev %x \n", __func__, ch, reg, ret, read_cmd(reg, ch));
...
static void sc16is7x2_dowork(struct sc16is7x2_channel *chan)
{
    ...
    if(!freezing(current))
    {
        ...
        //        printk("%s test i2c transfer completed \n", __func__);
...
static int sc16is7x2_handle_rx(struct i2c_client *client, unsigned ch)
{
    ...
    num_read = 0;
    do {
        lsr = sc16is7x2_read(client, UART_LSR, ch);
        if((lsr & 0x01))
        {
...
*/
    ...
    tty_insert_flip_string(tty, chan->buf, rxlvl);
    //    spin_lock_irqsave(&uart->lock, flags);
    /* Update RX counter */
#endif
...
static void sc16is7x2_handle_baud(struct i2c_client *client, unsigned ch)
{
    ...
    //    chan->lcr = sc16is7x2_read(client, UART_LCR, ch);
    //    sc16is7x2_write(client, UART_IER, ch, 1);
    ...
    sc16is7x2_write(client, UART_LCR, ch, 3/*chan->lcr*/);     /* reset DLAB */
...
{
    //    printk(&p->sc16is->client_dev->dev, "%s\n", __func__);
    //    sc16is7x2_write(client, UART_FCR, 0, UART_FCR_ENABLE_FIFO);
    ...
    //    sc16is7x2_write(client, UART_FCR, 0, 0);
}
...
static void sc16is7x2_handle_regs(struct i2c_client *client, unsigned ch)
{
    ...
    sc16is7x2_write(client, UART_LCR, ch, 0xBF); // access EFR register
    sc16is7x2_write(client, UART_EFR, ch, 0x10); // enable enhanced registers
    sc16is7x2_write(client, UART_LCR, ch, 0x03); // 8 data bit, 1 stop bit, no parity

    sc16is_clear_fifos(client, ch);
    //    sc16is7x2_write(client, UART_FCR, ch, chan->fcr);
    //    sc16is7x2_write(client, UART_MCR, ch, chan->mcr);
    //    sc16is7x2_write(client, UART_IER, ch, /*chan->ier*/1);
...
static void sc16is7x2_handle_channel(struct work_struct *w)
{
    ...
    while(1)
    {
        ...
        sc16is7x2_write(ts->client, REG_SPR, ch, 'H');
        if(sc16is7x2_read(ts->client, REG_SPR, ch) == 'H')
        {
            printk("%s test complited!!! \n", __func__);
        } else
            printk("%s test doesn't complite!!! \n", __func__);
...
//        sc16is7x2_handle_tx(ts->client, ch);
    ...
    printk("%s finished chan->iir %d, rxlvl %d \n", __func__, chan->iir, rxlvl);
}
...
static int sc16is7x2_startup(struct uart_port *port)
{
...

    {
        ...
        sc16is7x2_write(client, UART_EFR, ch, UART_EFR_ECB);
    ...
    chan->workqueue = create_freezable_workqueue(DRIVER_NAME);
    if (!chan->workqueue) {
    ...
    //     * one shot behaviour */
    if (request_irq(client->irq, sc16is7x2_irq,
    ...
    chan->ier = 0; // UART_IER_RLSI | UART_IER_RDI | UART_IER_THRI;
    chan->mcr = 0;
    chan->fcr = UART_FCR_ENABLE_FIFO; // | UART_FCR_CLEAR_RCVR | UART_FCR_CLEAR_XMIT; //0;
    chan->efcr = 0;//EFCR_RTSCON | EFCR_RTSINVER; // Enable RS-485. FIXME: user generic interface
