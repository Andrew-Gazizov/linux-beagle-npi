...
static void stmpe_work(struct work_struct *work)
{
    ...
    input_report_key(ts->idev, BTN_TOUCH, 0);//Pshka
...
static irqreturn_t stmpe_ts_handler(int irq, void *data)
{
    ...
    y = 4000 - (((data_set[1] & 0xf) << 8) | data_set[2]);
    z = data_set[3];
    ...
    input_report_key(ts->idev, BTN_TOUCH, 1);//Pshka
    input_sync(ts->idev);
...
/*
    ret = stmpe_set_bits(stmpe, STMPE_REG_TSC_CTRL,
                         OP_MODE(0xff), 0x01 | OP_MODE(OP_MOD_XY));
    if (ret) {
...
*/
    ...
    //
//    stmpe_set_bits(stmpe, stmpe->regs[STMPE_IDX_IER_LSB], 0xff, 0x01 | 0x02 | 0x04);
...
/*
    ret = stmpe_reg_write(stmpe, STMPE_REG_FIFO_TH, 0x01);
    if (ret) {
        dev_err(dev, "Could not set FIFO\n");
        return ret;
    }

    __stmpe_reset_fifo(stmpe);

    stmpe_set_bits(stmpe, 0x42,
                         0x7ff,
                         1);

    stmpe_set_bits(stmpe, 0x44,
                         0x7ff,
                         1);

    stmpe_set_bits(stmpe, 0x46,
                         0x7ff,
                         0);

    stmpe_set_bits(stmpe, 0x48,
                         0x7ff,
                         0);
*/
    ...
    stmpe_set_bits(stmpe, STMPE_REG_INT_STA,
                   0xff, 0xff);

    // Set interrupt mode and enable interrupt.
    stmpe_set_bits(stmpe, stmpe->regs[STMPE_IDX_ICR_LSB], 0xff, 0x02 |
                   0x01);
...
static int __devinit stmpe_input_probe(struct platform_device *pdev)
{
    ...
    ts_irq = platform_get_irq_byname(pdev, "FIFO_TH"); //FIFO_TH //TOUCH_DET
    if (ts_irq < 0)
        return ts_irq;
    ...
    request_irq(266, stmpe_ts_handler, IRQF_SAMPLE_RANDOM,
                      STMPE_TS_NAME, ts);

    ret = request_threaded_irq(ts_irq, NULL, stmpe_ts_handler,
                               IRQF_ONESHOT, STMPE_TS_NAME, ts);
//    ret = request_threaded_irq(266, stmpe_ts_handler, stmpe_ts_handler,
//                               IRQF_ONESHOT, STMPE_TS_NAME, ts);
    if (ret) {
    ...
    input_set_capability(idev, EV_KEY, BTN_TOUCH);//Pshka
