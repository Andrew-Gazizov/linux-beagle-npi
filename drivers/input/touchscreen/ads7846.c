...
static inline unsigned vbatt_adjust(struct ads7846 *ts, ssize_t v)
{
	...
	if (ts->model == 7845) // K=125/100 - коэффициент делителя Марка Епифанова
	{
		retval += retval << 2; /// (*5)
		retval >>= 2; /// (/4)
	}
...
static void ads7846_rx(void *ads)
{
	...
	if (Rt) {
		...
		input_report_abs(input, ABS_Y, 0xFFF - y);
...
static void ads7846_rx_val(void *ads)
{
	 ...
	 */
	val = (be16_to_cpup((__be16 *)t->rx_buf) & 0x7FFF) >> 3;
...
static int __devinit ads7846_probe(struct spi_device *spi)
{
	...
	err = ads7846_read12_ser(&spi->dev,
			  READ_12BIT_SER(vbatt) | ADS_PD10_ALL_ON);
	/// в ads7845 - vbat соответствует vaux
